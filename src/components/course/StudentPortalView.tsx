...
import {
  ...
  Award, Menu, X, RotateCcw, Loader2, Sparkles, ArrowLeft, Pencil,
} from "lucide-react";
...
export interface Flashcard { id: string; module_id: string; front: string; back: string; }
...
export interface PortalData {
  ...
  logoUrl: string | null; modules: PortalModule[];
}
...
// ── Shared portal view ──────────────────────────────────────────────────
// Renders the exact student experience. Used both by the public route
// (`/learn/:slug`, real students) and, in-place, by the course editor's
// "Visualizar como Aluno" mode (real course creator preview).
export interface StudentPortalViewProps {
  data: PortalData;
  /** Public link back to the course landing page. Omit when in creator preview. */
  exitHref?: string;
  /** Called when the creator wants to leave preview mode and go back to editing. */
  onExit?: () => void;
  /** True when rendered inside the course editor as a live preview (not a real student session). */
  previewMode?: boolean;
}

export function StudentPortalView({ data, exitHref, onExit, previewMode = false }: StudentPortalViewProps) {
  const [moduleIdx, setModuleIdx] = useState(0);
  const [completed, setCompleted] = useState<Set<string>>(new Set());
  const [sidebarOpen, setSidebarOpen] = useState(false);
  const [certOpen, setCertOpen] = useState(false);

  const progressKey = `${previewMode ? "portal_preview_progress_" : "portal_progress_"}${data.courseId}`;
  ...
  const saveProgress = useCallback((completedSet: Set<string>, currentModId: string) => {
  ...
  const goToModule = (idx: number) => {
    ...
    saveProgress(completed, data.modules[idx].id);
  };
    ...
    return (
      <div className="min-h-screen bg-[#0d1117] flex items-center justify-center text-center px-4">
        <div>
          ...
          <h1 className="text-xl font-bold text-white mb-2">Nenhum módulo publicado ainda</h1>
          <p className="text-slate-500">Adicione conteúdo ao curso para ver a experiência do aluno.</p>
        </div>
      </div>
    );
  }
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <span className="block">
                <Button
                  ...
                  style={{ backgroundColor: isAllDone && !previewMode ? "#eab308" : "#374151" }}
                  disabled={!isAllDone || previewMode}
            ...
            {previewMode && (
              <TooltipContent side="top">
                Certificados só são emitidos na visão pública real do aluno
              </TooltipContent>
            )}
          </Tooltip>
        </TooltipProvider>
    ...
    >
      ...
      {previewMode && (
        <div className="sticky top-0 z-50 flex items-center justify-center gap-2 h-9 bg-primary text-primary-foreground text-xs font-semibold px-4">
          ...
          Modo Visualização — você está vendo o curso como um aluno veria
          {onExit && (
            <Button
              size="sm"
              variant="secondary"
              className="h-6 ml-3 px-2 text-[11px] gap-1"
              onClick={onExit}
              data-testid="exit-preview-mode"
            >
      ...
      {/* Top header */}
      <header className={`sticky ${previewMode ? "top-9" : "top-0"} z-40 bg-[#161b22] border-b border-[#30363d] h-14 flex items-center px-4 gap-3`}>
        <button
        ...
        {exitHref && (
          <>
            <a href={exitHref} className="text-slate-500 hover:text-white transition-colors hidden sm:flex items-center gap-1.5 text-sm">
      ...
      <div className="flex flex-1 relative">
        ...
        <aside className={`
          fixed lg:sticky ${previewMode ? "top-[5.75rem]" : "top-14"} z-30 h-[calc(100vh-3.5rem)]
            ...
            >
              ...
              {module.quizQuestions.length > 0 && (
                <section className="mt-12">
                  ...
                  <QuizPlayer questions={module.quizQuestions} />
                </section>
              )}
                ...
                <div className="flex gap-3">
                  ...
                  {moduleIdx < modules.length - 1 ? (
                    ...
                    <Button
                      ...
                      onClick={() => { markComplete(module.id); if (!previewMode) setCertOpen(true); }}
      ...
      {certOpen && !previewMode && <CertModal courseId={data.courseId} onClose={() => setCertOpen(false)} />}
    </div>
  );
}
