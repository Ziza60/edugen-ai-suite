...
- **v5.5.9** (current) — **`[MODULE-CLASSIFY]` diagnostic log**. Single-line addition in `index.ts` per-module loop (after `getModuleRule` resolution): `[MODULE-CLASSIFY] mi=N title="…" kind=oop|fundamentals|… course="…"`. Lets us prove which `kind` every module actually got and catch first-match-wins misorderings in `PYTHON_MODULE_RULES` (e.g. "Introdução à POO" matching `fundamentals` regex before `oop` because fundamentals is listed first). No behavioral change. `ENGINE_VERSION = "5.5.9"`.
