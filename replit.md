...
- **v5.8.7** (current) — **regression-lock + 4 contract fixes**. ROOT CAUSE FIXED: `l2Replan CODE_TOO_LONG` was splitting repaired code at mid-point, putting `resultado = fn(...)` + `print(resultado)` on Slide B (2-3 lines) which then failed `isRenderableSlide` and was DROPPED — this was the slide 13 observable-outcome regression. FIX: detect blank-line + `resultado = ` sentinel in code; trim function body to `MAX_CODE_LINES − demoLines.length` to make room, emit `[REPAIR-PERSISTENCE] stage=l2_code_too_long demo_protected=true`. NEW `[REPAIR-PERSISTENCE] stage=post_guardrail4` log confirms observable demo is PRESENT/ABSENT after Guardrail 4. CONTRACTS: (1) `json_apis` gets `http_post` contract (`requests.post(`) + `synthesizeForContract("http_post")` canonical — POST + status_code + json() + print; `"http_post"→"Requisição POST com requests"` in CONTRACT_TITLE_MAP. (2) `files_exceptions` gets `file_write` contract (`open("w")|.write(`) + `synthesizeForContract("file_write")` write+read example — resolves near-duplicate slides 24/25 by injecting distinct write slide. (3) `data_structures/collection_ops` requiredPatterns now includes `/print\s*\(/` — slides without output no longer satisfy the contract, forcing injection of canonical example with `print`. (4) No changes to planner, renderer, qaVeto, billing, or visual pipeline. ENGINE_VERSION="5.8.7".
- **v5.8.6** (previous) — **3 polish fixes: CLI truncation, archetype cleanup, módulo 3 prático**. (1) NEW Guardrail 1f — detecta `pip install -r…` e outros comandos CLI com truncação nos bullets e repara para forma canônica (`pip install -r requirements.txt`); adicionado `pip install` ao `SQL_STAR_PRESERVE_RE` para bloquear truncações futuras em `safeSliceText`. Emite `[VISUAL-TRUNCATION-FIX]`. (2) NEW Guardrail 1e — strips trailing leaked list-numbers de bullet items (e.g. `biblioteca = [] 2`); quando maioria dos items são assignment-like (código Python vazado em bullets), converte o slide para layout `code`. Emite `[ARCHETYPE-CLEANUP]`. (3) NEW `data_structures` entry em `MODULE_SKILL_CONTRACTS` com contrato `collection_ops` (requiredPatterns: `.append(` + `["` + `set()|.add(`); `synthesizeForContract("collection_ops")` retorna exemplo canônico com lista + dict + tuple + set + append + lookup por chave. `CONTRACT_TITLE_MAP`: `"collection_ops" → "Operações com Coleções em Python"`. Módulo 3 receberá snippet real se ausente. Emite `[MODULE-PRACTICE-DENSITY]` via contrato inject. ENGINE_VERSION="5.8.6".
- **v5.8.5** (previous) — **4 refinamentos: accumulator return, title-snippet semântico, cobertura módulo 7 (3 snippets), densidade PROCESSO**. (1) HARDENED `function_defined_but_uncalled` repair — antes de appendar a chamada, inspeciona o corpo da função em busca de último assignment sem `return`. Se o padrão accumulator (loop + `+=` + `total|liquido|bruto|soma|contador`) for detectado, insere `return <lastVar>` imediatamente antes da chamada demo. Cobre o caso canônico do slide 14: `def calcularvalortotal(carrinho): ... totalliquido = totalbruto - desconto` → repara para `return totalliquido` + `resultado = calcularvalortotal([...])` + `print(resultado)`. Emite `[OBSERVABLE-OUTCOME-TRACE] detected=accumulator_pattern|function_no_return`. (2) NEW `detectTitleSnippetMismatch(title, code)` + NEW `TECH_ENTITY_PATTERNS` (pdb / unittest / pytest / logging / requests) — detecta title_entities vs code_entities e flags quando o título diz "unittest" mas o código tem só `pdb`. Novo **Guardrail 8b** em `applyFinalGuardrails` reescreve o título para o canonical da tecnologia real (`"Depuração com pdb"`). Emite `[TITLE-SNIPPET-ALIGNMENT] status=FAILED title_rewritten=...`. (3) EXPANDIDO `MODULE_SKILL_CONTRACTS.tests_logs` com contrato `pdb_debug` (requiredPatterns: `/\bpdb\b|\.set_trace/`). NOVO `synthesizeForContract(contractName, ...)` — síntese canônica direta por nome de contrato (bypassa keyword-guessing do título): `pdb_debug`→pdb+set_trace, `logging_emit`→basicConfig+info+warning, `unittest_or_pytest`→TestCase+assertEqual. CHANGED injeção de skill contracts: de "1 slide para 1º contrato" para **loop sobre TODOS os contratos faltantes** (um slide por contrato) — módulo 7 agora recebe unittest + logging + pdb se todos os 3 estiverem ausentes. Adicionado `"pdb_debug": "Depuração com pdb"` ao `CONTRACT_TITLE_MAP`. (4) NEW **Guardrail 1d** — slides com `layout=process` e `items.length > 5` → trimados para 5 itens; emite `[PROCESS-DENSITY] items=N → trimmed to 5`. Sem regressão: HTTP parity, anti-demotion, narrative alignment, qaVeto, billing intocados. ENGINE_VERSION="5.8.5".
- **v5.8.4** (previous) — **4 quality improvements: Narrative Alignment, Observable Outcome hardening, Title Editorial Quality, Title↔Code Coherence**. (1) NEW `detectNarrativeAlignment(slide)` — for OOP code slides, extracts class names from code and significant nouns from items/bullets; checks 4-char-stem overlap; flags mismatch (e.g. text="Cachorro/raça/latir" vs code="class Carro/marca"). NEW `synthesizeAlignedCode(textEntities, moduleKind, ctx)` — when mismatch is detected, synthesizes OOP code where the class name, attributes and methods are derived from the text entities (verb endings `ar/er/ir` → methods; remaining nouns → attributes). Fires as **Guardrail 7** in `applyFinalGuardrails` (OOP modules only). Emits `[NARRATIVE-ALIGNMENT]` log with status=FAILED/REPAIRED/UNREPAIRED. (2) NEW bare_def_stub repair in `repairIncompleteCodeExample` — triggers on `trailing_open_bracket` or `too_few_code_lines` when the entire code is a single `def fname(params):` line (no body). Builds complete function body + call based on name heuristics: `tests_logs`/`log` → adds logging.basicConfig + logging.info f-string + return + print call; `soma/sum/total` → sum() body; `media/mean` → sum()/len() body; generic → str().upper() body. Emits `[CODE-REPAIR] pattern=bare_def_stub`. (3) NEW `rewriteTitleFromCode(title, code, moduleKind, slideNum)` — detects "X or Y" / "X_or_Y" / "Exemplo: contract_name" placeholder titles and rewrites them to professional Portuguese based on actual code content: `pdb`→"Depuração com pdb", `unittest`→"Testes com unittest", `logging.*`→"Logging de Operações", type hints→"Docstrings e Type Hints", etc. Fires as **Guardrail 8** on every slide. Emits `[EDITORIAL-TITLE]` log. (4) NEW `CONTRACT_TITLE_MAP` — 9-entry lookup replacing raw contract identifiers ("unittest_or_pytest") with editorial Portuguese titles at the skill-contract injection site, before Guardrail 8 even runs. Fallback: `_or_` → " e " + title-case. ADDED `NARRATIVE_ALIGNMENT_FAILURE` to `QAIssueType`. No regressions: HTTP parity (Guardrail 6, GET+POST+json()) unchanged; planner, renderer, qaVeto, billing untouched; ENGINE_VERSION="5.8.4".
- **v5.7.1** (previous) — **Code completeness = pedagogical demonstration, not just "runnable"**. Diagnosed 4 production slides (13/35/43/49 of "Introdução à Programação em Python") that passed `validateSemanticCodeCompleteness` despite being didactically broken. Root cause = 3 escapes in the validator, all fixed: (1) NEW rule **`function_defined_but_uncalled`** — top-level `def name(...)` with no call site `name(...)` outside its signature returns this reason; decorated defs (`@deco\ndef fn`) and decorator references (`@fn`) count as use; recursion in body counts (acceptable corner case). (2) HARDENED **`class_instance_no_method_call`** replaces the bare `print(<inst>)` escape on the old `class_defined_but_unused` rule — instantiation alone is no longer sufficient; at least one instance MUST do `inst.method(...)` or `inst.attr` access (`print(obj)` showing object address is dropped). Old rule preserved as fallback `class_defined_but_no_output`. (3) NEW preventive helper `repairBareInstancePrint(code, moduleCtx, slideNum)` — extracted the print-inst-to-method rewrite from `repairIncompleteCodeExample` (where it was chicken-and-egg gated on validation already failing) and promoted it to a new **Guardrail 3b** in `applyFinalGuardrails`, BEFORE the validator runs. Method picked from local funcs first, then `moduleCtx.funcs`, skipping `__init__/__new__/__str__/__repr__/main/init`. Emits `[CODE-PREVENTIVE-REPAIR]` log. (4) NEW repair pattern `function_call_demo` in `repairIncompleteCodeExample` — when reason is `function_defined_but_uncalled`, appends `resultado = fnName(args)\nprint(resultado)` with args inferred from parameter names (lista→`[10,20,30]`, preco/valor/num/idade→`10`, nome/texto→`"exemplo"`, path→`"dados.txt"`, url→`"https://api.example.com"`, flag/is_*→`True`, fallback→param name as string). Class repair guard at L747 extended to also handle the new `class_instance_no_method_call` and `class_defined_but_no_output` reasons. (5) Tests `__tests__/v571_code_completeness.smoke.ts` covering all 4 user-reported regression cases + 5 positive controls (function with call demo, class with method/attr access, decorator pattern, methods-only inside class, legacy ellipsis/too_few). 9/9 verified via Node V8 (deno test path: `deno test --allow-read supabase/functions/export-pptx-v4/__tests__/v571_code_completeness.smoke.ts`). All restrictions honored: no frontend, no renderer, no planner edits; v4/v5.7 remains canonical engine; QA veto active; no CODE→bullets demote to hide problems — repair completes the example. `ENGINE_VERSION = "5.7.1"`.
