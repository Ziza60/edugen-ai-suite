...
const PISO_CONTEUDO = floorMinContent("standard");
const hollow: DeckModule[] = [
  ...
  // "Saudável" quer dizer: já está NO piso, então o backfill não tem o que
  // fazer. O piso não é fixo — é floorMinContent(density), hoje DENSITY_SPECS
  // .standard.min - 1 = 5. Quando este teste foi escrito o piso era 2 e três
  // slides bastavam; a densidade subiu depois e o módulo virou carente sem que
  // o teste percebesse. É por isso que ele conta cinco agora, e por isso a
  // contagem vem de floorMinContent e não de um número solto: no dia em que a
  // densidade mudar de novo, o fixture acompanha em vez de acusar regressão.
  {
    title: "Módulo Saudável",
    slides: [
      ...Array.from({ length: PISO_CONTEUDO }, (_, i) => ({
        kind: "bullets" as const,
        title: `Seção ${i + 1}`,
        bullets: [`ponto ${i + 1}a`, `ponto ${i + 1}b`],
      })),
