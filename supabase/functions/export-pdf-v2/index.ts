...
class R {
  ...
  table(rawLines: string[]) {
    ...
    for (const row of rowData) {
      ...
      const capMm = (SIZE * 0.7) / PT; // approx cap height in mm
      ...
      if (lv > 0) {
        ...
        while (k < lines.length) {
          ...
          if (lv2 > 0) {
            ...
            cascade +=
              (lv2 === 2 ? SP.B_H2 : lv2 === 3 ? SP.B_H3 : SP.B_H4) +
              lhMm(s2, 1.25) +
...
serve(async (req: Request) => {
  ...
  try {
    ...
    await serviceClient
      ...
      .insert({
        user_id: user.id,
        event_type: "COURSE_EXPORTED_PDF_V2",
