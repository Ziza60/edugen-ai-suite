// export-pdf-v2/index.ts  — BUILD 2026-06-22c
// Fixes vs 2026-06-22b:
//  [1] Module title clamp off-by-one: check `ty > limit` (current position)
//      instead of `ty + lineAdvMm > limit` (lookahead that skipped the last line)
//  [2] Cover description: removed .slice(0,5) cap; rely on mm safety cap instead
//  [3] Paragraph collector: break at sentence-ending punctuation (.!?)
//      so "Contexto/Desafio/Solução/Resultado" items stay separate paragraphs
//      while hard-wrapped continuation lines still get joined and justified
//  [4] Table pre-check: use remaining space on current page, not fixed fullPageH
//  [5] MIN_KEEP reduced 40 → 30 mm — less aggressive page-break on headings
//  [6] lhMm formula corrected: use pts→mm conversion for heading line height
...
// pts → mm line advance helper
function lhMm(sizePt: number, factor = 1.3): number {
...
// A line is "special" (not plain paragraph text)
...
// [FIX #3] A source line that ends with sentence-terminating punctuation
// is a complete semantic unit — the next line starts a new paragraph.
function endsWithSentence(line: string): boolean {
...
class R {
  ...
  cover(title: string, description?: string) {
    ...
    // [FIX #2] Description — no .slice(0,5) limit; rely on mm cap
    if (description) {
      ty += 6;
      const dLines = wrapText(safeText(description), this.reg, FS.COVER_SUB, PW - 60 * PT);
      for (const line of dLines) {
        if (ty > 210) break; // safety cap: don't overlap bottom bar
  ...
  // ── Module page — banner at top of content page ──
  modulePage(title: string, num: number) {
    ...
    // Module title — [FIX #1] clamp check: `ty > limit` not `ty + advance > limit`
    const tLines    = wrapText(safeText(title), this.bld, FS.MOD_TITLE, PW - 50 * PT);
    const titleAdv  = lhMm(FS.MOD_TITLE, 1.3);   // mm per title line
    const CLAMP_Y   = MOD_BAN_H - 3;              // max baseline = 41 mm
    let ty = 26;
    for (const line of tLines) {
      if (ty > CLAMP_Y) break;                    // [FIX #1] current position check
  ...
  // ── Paragraph — justified with exact font metrics ──
  para(text: string) {
    ...
    // Anti-widow: if multi-line para but ≤1 line fits here, move to next page
  ...
  heading(text: string, level: number, keepH = 0) {
    ...
    const adv   = lhMm(size, 1.25);   // [FIX #6] correct pts→mm
    const lines = wrapText(clean, this.bld, size);
    this.check(bef + lines.length * adv + aft + (level === 2 ? 2 : 0) + keepH);
    this.y += bef;
  ...
  table(rawLines: string[]) {
    ...
    // [FIX #4] Pre-check using ACTUAL remaining space, not fixed fullPageH
    const totalH      = rowData.reduce((s, r) => s + r.rowH, 0);
    const remaining   = MAX_Y - this.y;
    const freshPageH  = MAX_Y - MT;   // space on a fresh normal page

    if (totalH <= freshPageH && totalH > remaining) {
      // Whole table fits on one page but not here → move to fresh page
      this.addPage();
    } else if (totalH > freshPageH) {
      // Table taller than a full page — ensure at least 2 rows start here
      const twoRowH = rowData.slice(0, 2).reduce((s, r) => s + r.rowH, 0);
      if (twoRowH > remaining) this.addPage();
    }

    // Track segments for vertical dividers (single-page tables only)
    let segStart  = this.y;
    let multiPage = false;
    ...
    // Vertical column dividers (single-page tables only)
    if (!multiPage) {
      const tblEnd = this.y;
      for (let c = 1; c < numCols; c++) {
        const divX = ML_PT + c * colW_pt;
        this.pg.drawLine({
          start: { x: divX, y: this.Y(tblEnd) },
  ...
  content(markdown: string) {
    ...
    while (i < lines.length) {
      ...
      // Heading — [FIX #5] MIN_KEEP reduced to 30 mm
      ...
      if (lv > 0) {
        ...
        while (k < lines.length) {
          ...
          const t2 = lines[k].trim(), lv2 = headingLevel(t2);
          if (lv2 > 0) {
      ...
      // [FIX #3] Plain paragraph:
      // - Collect consecutive non-special lines
      // - BREAK at sentence-ending punctuation (.!?) so that labeled items
      //   (Contexto/Desafio/Solução/Resultado) remain separate paragraphs
      // - Lines NOT ending with sentence punct are continuation (hard-wrapped)
      //   and get joined for proper justification
      listN = 0;
      const paraLines: string[] = [t];
      i++;
      // Only collect more lines if the first line does NOT end with sentence punct
      if (!endsWithSentence(t)) {
        while (i < lines.length) {
          ...
          paraLines.push(next);
          i++;
          if (endsWithSentence(next)) break; // sentence complete → flush
        }
      }
